...
def show_project(request, project_name):
    ...
    if not cached_project_context:
        ...
        designs = [
            design for design in Design.objects.filter(design__project=project_name)
        ]

        works_without_current_project = [work for work in works if work != project]

        if len(works_without_current_project) < 3:
            # If there are fewer than 3 works, use the entire list
            recommendation = works_without_current_project
        else:
            # Sample 3 works
            recommendation = random.sample(works_without_current_project, k=3)

        paginator = Paginator(recommendation, 3)
        per_page = request.GET.get("page")
        recommendation_page = paginator.get_page(per_page)
...
def contact_us(request):
    ...
    if request.htmx:
        if request.method == "POST":
            ...
            messages.warning(
                request,
                "Holly molly 😱, something goes wrong ... please re submit your GOGO.")
...
def email_content(email, client_subject, client_message, website):
    ...
    html_message = render_to_string("Components/Letter.html", context)
